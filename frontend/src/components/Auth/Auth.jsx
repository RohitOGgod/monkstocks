...
const Auth = () => {
  ...
  const handleSubmitGuestAccount = (e) => {
    ...
    dispatch({ type: AUTH_ERROR_OCCURRED, payload: "" });
    try {
      const encEmail = process.env.REACT_APP_GUEST_EMAIL;
      const encPass = process.env.REACT_APP_GUEST_PASS;
      if (!encEmail || !encPass) {
        setIsLoadingGuest(false);
        return dispatch({ type: AUTH_ERROR_OCCURRED, payload: "Guest account is not configured. Please set REACT_APP_GUEST_EMAIL and REACT_APP_GUEST_PASS in frontend/.env (base64-encoded)." });
      }
      const email = atob(encEmail);
      const password = atob(encPass);
      if (!email || !password) {
        setIsLoadingGuest(false);
        return dispatch({ type: AUTH_ERROR_OCCURRED, payload: "Invalid guest credentials. Ensure they are base64-encoded in frontend/.env." });
      }
      dispatch(loginUser({ email, password }, history, state));
    } catch (err) {
      setIsLoadingGuest(false);
      dispatch({ type: AUTH_ERROR_OCCURRED, payload: "Failed to decode guest credentials. Please verify the base64 values in frontend/.env." });
    }
  };
