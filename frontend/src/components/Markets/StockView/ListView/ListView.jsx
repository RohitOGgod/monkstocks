              ...
              <>
                {filteredStocks.map((stock) => (
                  <tr key={stock._id} className="bg-white dark:bg-gray-700 hover:bg-gray-100 dark:hover:bg-gray-600">
                    ...
                    <td className="hidden md:table-cell px-5 py-5 border-b border-gray-200 dark:border-gray-800 text-sm">
                      <div className="flex items-center">
                        <img
                          className="h-8 w-8 rounded bg-gray-100 dark:bg-gray-800 mr-3 object-contain"
                          src={stock.icon}
                          alt={stock.name}
                          onError={(e) => { e.currentTarget.onerror = null; e.currentTarget.src = `https://via.placeholder.com/32?text=${encodeURIComponent(stock.ticker || 'S')}`; }}
                          onClick={() => detailPage(stock._id)}
                        />
                        <div className="flex items-center space-x-2">
                          <p onClick={() => detailPage(stock._id)} className="text-gray-900 dark:text-white whitespace-no-wrap cursor-pointer">
                            {stock.name}
                          </p>
