...
const StockView = () => {
  // Resolve backend URL for sockets
  const socketURL = useMemo(() => {
    const envUrl = process.env.REACT_APP_STOCKS_API && process.env.REACT_APP_STOCKS_API.trim();
    return envUrl || api?.defaults?.baseURL || 'http://localhost:5000';
  }, []);

  // Keep a single socket instance for the component lifetime
  ...
  useEffect(() => {
    ...
    if (!socketRef.current) {
      socketRef.current = socketIOClient(socketURL, { transports: ['websocket', 'polling', 'flashsocket'] });
