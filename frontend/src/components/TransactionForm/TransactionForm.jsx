...
const initialState = { stockId: null, sharesBought: 1 };
...
const TransactionForm = () => {
  ...
  // Ensure form has stockId when param is available
  useEffect(() => {
    if (id && !form.stockId) {
  ...
  const handleSubmitNewPurchase = (e) => {
    e.preventDefault();
    dispatch({ type: PURCHASED_ERROR_OCCURRED, payload: "" });
    const qty = Number(form.sharesBought ?? shares);
    if (!qty || qty < 1 || qty > 100) {
      return dispatch({ type: PURCHASED_ERROR_OCCURRED, payload: "Enter between 1 and 100 shares." });
    }
    const payload = { stockId: form.stockId ?? id, sharesBought: qty };
    dispatch(addPurchase(payload, history));
  ...
  const handleChange = (e) => {
    ...
    setShares(val);
    setForm({ ...form, [e.target.name]: val, stockId: id });
  }
    ...
    !stock || !stock._id ?
      <div className="bg-white dark:bg-gray-800 w-full h-full text-center">
        <div className="max-w-2xl mx-auto py-64 bg-white dark:bg-gray-800 flex items-center justify-center">
          <div className="p-6">
            <h1 className="block mt-2 text-2xl font-semibold text-gray-800 dark:text-white">Loading transaction...</h1>
            <div className="mt-8">
                  ...
                  <div className="flex flex-col items-center px-4 py-3 text-gray-700 rounded-md dark:text-gray-200 ">
                    <div>
                      <span className="relative inline-block px-3 py-1 font-semibold text-blue-900 leading-tight">
                        ...
                        <span className="relative text-blue-500 dark:text-blue-400">
                          ${Number(shares) && stock?.currentPrice ? parseFloat(Number(shares) * Number(stock.currentPrice)).toFixed(2) : (0).toFixed(2)}
                        </span>
                      </span>
                    </div>
